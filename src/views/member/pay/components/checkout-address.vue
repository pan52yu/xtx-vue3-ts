<script lang="ts" name="CheckoutAddress" setup>
...
<template>
    <div class="checkout-address">
        <div class="text">
            <div v-if="!checkout.showAddress" class="none">
                您需要先添加收货地址才可提交订单。
            </div>
            <template v-else>
                <ul>
        ...
        <XtxDialog v-model:visible="dialogVisible" title="添加收货地址">
        ...
        <XtxDialog v-model:visible="visible" title="切换收货地址">
            <template #default>
                <div
                    v-for="item in checkout.checkoutInfo.userAddresses"
                    :key="item.id"
                    :class="{ active: selectAddress === item }"
                    class="text item"
